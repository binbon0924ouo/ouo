...
package net.mcreator.ouo.keybind;
...
public class ConversionKeyBinding extends OuoModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public ConversionKeyBinding(OuoModElements instance) {
		super(instance, 19);
	...
	public void initElements() {
		keys = new KeyBinding("key.ouo.conversion", GLFW.GLFW_KEY_G, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					OuoMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
